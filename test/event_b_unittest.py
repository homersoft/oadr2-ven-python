# This file is like the other event_unittest.py, but for 2.0b unit tests
# NOTE: test_batch_a/b/c()'s sample XML payloads come from the 2.0a unit tests, just with the renamed spec
#       this should be temporary though.
...
class EventHandlerTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config = {'vtn_ids': 'vtn_1,vtn_2,vtn_3,TH_VTN,VTN_543',
                       'ven_id': VEN_ID,
                       'oadr_profile_level': event.OADR_PROFILE_20B}
        with open(os.path.join(SCHEMA_DIR, 'oadr_20b.xsd')) as oadr_schema_file:
    ...
    def test_build_request_payload(self):
        ...
        ven_id = payload.findtext('pyld:eiRequestEvent/ei:venID', namespaces=event.NS_B)
    ...
    def test_build_error_response(self):
        ...
        err_code = payload.findtext('pyld:eiCreatedEvent/ei:eiResponse/ei:responseCode', namespaces=event.NS_B)
        ven_id = payload.findtext('pyld:eiCreatedEvent/ei:venID', namespaces=event.NS_B)
        print('Testing VEN ID')
        self.assertEqual(ven_id, VEN_ID,
                         msg='VEN ID for generated payload did not match; ven_id=%s, VEN_ID=%s' % (ven_id, VEN_ID))
        print('OK')
        print('Testing Error Code')
        self.assertEqual(err_code, code,
                         msg='Error code for generated payload did not match; err_code=%s, code=%s' % (err_code, code))
        print('OK')
    ...
    def test_build_created_payload(self):
        ...
        i = 0
        for res in payload.iterfind('pyld:eiCreatedEvent/ei:eventResponses/ei:eventResponse', namespaces=event.NS_B):
            # Yank data
            res_code = res.findtext('ei:responseCode', namespaces=event.NS_B)
            res_id = res.findtext('pyld:requestID', namespaces=event.NS_B)
            e_id = res.findtext('ei:qualifiedEventID/ei:eventID', namespaces=event.NS_B)
            mod_num = res.findtext('ei:qualifiedEventID/ei:modificationNumber', namespaces=event.NS_B)
            opt = res.findtext('ei:optType', namespaces=event.NS_B)

            # Run the assertion tests
            print('Event: ' + events[i][0])
            self.assertEqual(events[i][0], e_id,
                             msg='Event ID is not the same; desired=%s, got=%s' % (events[i][0], e_id))
            self.assertEqual(events[i][1], mod_num,
                             msg='Mod Number is not the same; desired=%s, got=%s' % (events[i][1], mod_num))
            self.assertEqual(events[i][2], res_id,
                             msg='Request ID does not match up; desired=%s, got=%s' % (events[i][2], res_id))
            self.assertEqual(events[i][3], opt,
                             msg='Opt status is not the same; desired=%s, got=%s' % (events[i][3], opt))
            self.assertEqual(events[i][4], res_code,
                             msg='Status code is not the same; desired=%s, got=%s' % (events[i][4], res_code))
    ...
    def test_handle_payload(self):
        ...
        files = ['Sample_oadrDistributeEvent.xml',  # First file generates validation errors, due to 'description' field
                 'Sample_oadrDistributeEvent_W_Baseline.xml',
                 'Sample_oadrDistributeEvent_W_GML.xml'
                 ]
    ...
    def test_batch_c(self):
        ...
        for i in range(1, 9):
            ...
            if payload is not None:
                #                print(etree.tostring(payload, pretty_print=True))
                # Get some data
                status_code = payload.findtext('pyld:eiCreatedEvent/ei:eventResponses/ei:eventResponse/ei:responseCode',
                                               namespaces=event.NS_B)
                opt_type = payload.findtext('pyld:eiCreatedEvent/ei:eventResponses/ei:eventResponse/ei:optType',
                                            namespaces=event.NS_B)
